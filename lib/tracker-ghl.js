...
async function linkReferral(target, referrer, note) {
  ...
  // Resolve the End of Funnel destination up front (best-effort) so we can both
  // record where each opportunity came from and move them there.
  let dest = null;
  let moveWarning = '';
  try {
    dest = await resolveEndOfFunnelTarget();
  } catch (error) {
