...
function pickReferralTrackingDestination(target, referrer) {
  const cfg = getGhlConfig();
  return {
    pipelineId: cfg.referralPipelineId || referrer.pipelineId || target.pipelineId,
    pipelineStageId: cfg.referralPipelineStageId || referrer.pipelineStageId || target.pipelineStageId,
...
async function createReferralTrackingOpportunity(target, referrer, note) {
  ...
  if (!referrer.contactId) {
    const err = new Error('Referrer contact ID is missing, so a tracking opportunity cannot be created.');
    err.statusCode = 400;
    throw err;
  }
...
async function createOrReuseReferralTrackingOpportunity(target, referrer, note) {
  if (target.referralTrackingOpportunityId && (!target.referredById || target.referredById === referrer.id)) {
    return {
      id: target.referralTrackingOpportunityId,
      reused: true,
      name: `Referral - ${target.name}`,
      contactId: referrer.contactId,
    };
...
async function linkReferral(target, referrer, note) {
  ...
  const trackingOpportunity = await createOrReuseReferralTrackingOpportunity(target, referrer, note);
  const linkedAt = new Date().toISOString();
  const auditNote = referralLinkMessage(target, referrer, note);
  const referralUpdate = await updateOpportunityFields(target.id, {
    referrerOpportunityId: referrer.id,
    referrerName: referrer.name,
    referrerContactId: referrer.contactId,
    referralLinkedAt: linkedAt,
    referralNote: auditNote,
  ...
  const contactNote = await createContactNote(target.contactId, 'Referral updated by tracker', auditNote);
