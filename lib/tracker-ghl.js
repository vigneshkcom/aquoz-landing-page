...
function getGhlConfig() {
  return {
    ...
    referralPipelineId: process.env.AQUOZ_REFERRAL_TRACKING_PIPELINE_ID || '',
    referralPipelineStageId: process.env.AQUOZ_REFERRAL_TRACKING_STAGE_ID || '',
    referralTrackingStatus: process.env.AQUOZ_REFERRAL_TRACKING_STATUS || 'open',
  };
}
...
async function deleteOpportunity(opportunityId) {
  if (!opportunityId) return { skipped: true, reason: 'No opportunity ID was provided.' };
  try {
    return await ghlRequest(`/opportunities/${encodeURIComponent(opportunityId)}`, {
      method: 'DELETE',
    });
  } catch (error) {
    const notFound = error.statusCode === 404 || /not found/i.test(error.message || '');
    if (notFound) return { skipped: true, reason: 'Tracking opportunity was already missing.' };
    throw error;
  }
}
  ...
  for (const opportunity of won) {
    if (opportunity.referredById && byId.has(opportunity.referredById)) {
      byId.get(opportunity.referredById).referralsMade.push({
        ...
        trackingOpportunityId: opportunity.referralTrackingOpportunityId,
      });
    }
  }
...
function pickReferralTrackingDestination(target, referrer) {
  const cfg = getGhlConfig();
  return {
    pipelineId: cfg.referralPipelineId || target.pipelineId || referrer.pipelineId,
    pipelineStageId: cfg.referralPipelineStageId || target.pipelineStageId || referrer.pipelineStageId,
    status: cfg.referralTrackingStatus || 'open',
  };
}

async function createReferralTrackingOpportunity(target, referrer, note) {
  const cfg = assertGhlConfig();
  const { fields } = await ensureReferralFields();
  const destination = pickReferralTrackingDestination(target, referrer);
  const customFields = buildCustomFieldPayload({
    referrerOpportunityId: referrer.id,
    referrerName: referrer.name,
    referrerContactId: referrer.contactId,
    referredCustomerOpportunityId: target.id,
    referredCustomerName: target.name,
    referredCustomerContactId: target.contactId,
    referralLinkedAt: new Date().toISOString(),
    referralNote: note,
  }, fields);

  if (!target.contactId) {
    const err = new Error('Customer contact ID is missing, so a tracking opportunity cannot be created.');
    err.statusCode = 400;
    throw err;
  }
  if (!destination.pipelineId || !destination.pipelineStageId) {
    const err = new Error('Referral tracking pipeline or stage could not be resolved.');
    err.statusCode = 400;
    throw err;
  }
  ...
  const body = {
    ...
    pipelineStageId: destination.pipelineStageId,
    name: `Referral - ${target.name}`,
...
async function createOrReuseReferralTrackingOpportunity(target, referrer, note) {
  if (target.referralTrackingOpportunityId && (!target.referredById || target.referredById === referrer.id)) {
    let existing = null;
    try {
      existing = await fetchOpportunity(target.referralTrackingOpportunityId);
    } catch {
      existing = null;
    }
    if (existing && opportunityContactId(existing) === target.contactId) {
      return {
        ...existing,
        id: existing.id || target.referralTrackingOpportunityId,
        reused: true,
      };
    }
    if (existing) {
      await deleteOpportunity(target.referralTrackingOpportunityId);
    }
  }

  const opportunity = await createReferralTrackingOpportunity(target, referrer, note);
  return {
    ...opportunity,
...
async function linkReferral(target, referrer, note) {
  ...
  const trackingOpportunity = await createOrReuseReferralTrackingOpportunity(target, referrer, context);
  ...
  const referralUpdate = await updateOpportunityFields(target.id, {
    ...
    referralTrackingOpportunityId: trackingOpportunity.id,
    referralTrackingCreatedAt: trackingOpportunity.reused ? target.referralTrackingCreatedAt : linkedAt,
  });
...
async function unlinkReferral(target) {
  await ensureReferralFields();
  const deletedTrackingOpportunity = await deleteOpportunity(target.referralTrackingOpportunityId);
