...
const FIELD_DEFS = {
  ...
  // Where each opportunity sat before it was moved to End of Funnel on link,
  // so Unlink can put them back. Both are stored on the referred customer's
  // (target) opportunity, which owns the whole referral record.
  referralPreviousPipelineId: {
    env: ['AQUOZ_REFERRAL_PREVIOUS_PIPELINE_FIELD_ID'],
    names: ['Referral - Previous Pipeline ID', 'Referral Previous Pipeline ID'],
    dataType: 'TEXT',
  },
  referralPreviousStageId: {
    env: ['AQUOZ_REFERRAL_PREVIOUS_STAGE_FIELD_ID'],
    names: ['Referral - Previous Stage ID', 'Referral Previous Stage ID'],
    dataType: 'TEXT',
  },
  referrerPreviousPipelineId: {
    env: ['AQUOZ_REFERRER_PREVIOUS_PIPELINE_FIELD_ID'],
    names: ['Referral - Referrer Previous Pipeline ID', 'Referrer Previous Pipeline ID'],
    dataType: 'TEXT',
  },
  referrerPreviousStageId: {
    env: ['AQUOZ_REFERRER_PREVIOUS_STAGE_FIELD_ID'],
    names: ['Referral - Referrer Previous Stage ID', 'Referrer Previous Stage ID'],
    dataType: 'TEXT',
  },
};
...
function getGhlConfig() {
  return {
    ...
    // The "End of Funnel" pipeline has two stages. IDs win if set; otherwise the
    // pipeline/stages are matched by name (case-insensitive).
    ...
    // Stage a referral link moves both opportunities into.
    referredStageId: process.env.AQUOZ_REFERRED_STAGE_ID || process.env.AQUOZ_END_OF_FUNNEL_STAGE_ID || '',
    referredStageName: process.env.AQUOZ_REFERRED_STAGE_NAME || process.env.AQUOZ_END_OF_FUNNEL_STAGE_NAME || 'Referred Opportunities',
    // Stage a customer moves into once the review + referral email is sent.
    referralEmailSentStageId: process.env.AQUOZ_REFERRAL_EMAIL_SENT_STAGE_ID || '',
    referralEmailSentStageName: process.env.AQUOZ_REFERRAL_EMAIL_SENT_STAGE_NAME || 'Referral Email Sent',
    // Contact tag added when the review + referral email is sent.
    reviewSentTag: process.env.AQUOZ_REVIEW_SENT_TAG || 'Review email sent',
  };
}
...
function normalizeOpportunity(opportunity, referralFields, stageMap) {
  ...
  return {
    ...
    referralPreviousPipelineId: getCustomFieldValue(opportunity, referralFields.referralPreviousPipelineId),
    referralPreviousStageId: getCustomFieldValue(opportunity, referralFields.referralPreviousStageId),
    referrerPreviousPipelineId: getCustomFieldValue(opportunity, referralFields.referrerPreviousPipelineId),
    referrerPreviousStageId: getCustomFieldValue(opportunity, referralFields.referrerPreviousStageId),
    referrer: null,
    referralsMade: [],
...
// Find the "End of Funnel" pipeline (env ID wins; otherwise match by name).
async function resolveEndOfFunnelPipeline() {
  const cfg = getGhlConfig();
  const pipelines = await fetchPipelines();
  let pipeline = null;
  if (cfg.endOfFunnelPipelineId) pipeline = pipelines.find((p) => p.id === cfg.endOfFunnelPipelineId);
  if (!pipeline) {
    const want = normalizeName(cfg.endOfFunnelPipelineName);
    pipeline = pipelines.find((p) => normalizeName(p.name) === want)
      || pipelines.find((p) => normalizeName(p.name).includes(want));
  }
  if (!pipeline) {
...
function findStage(pipeline, stageId, stageName) {
  const stages = pipeline.stages || [];
  if (stageId) {
    const byId = stages.find((s) => s.id === stageId);
    if (byId) return byId;
  }
  const want = normalizeName(stageName);
  return stages.find((s) => normalizeName(s.name) === want)
    || stages.find((s) => normalizeName(s.name).includes(want))
    || null;
}

// Resolve a destination in the End of Funnel pipeline.
//   kind 'referred' -> the stage a linked referral moves into
//   kind 'emailed'  -> the stage a customer moves into once the email is sent
async function resolveEndOfFunnelStage(kind) {
  const cfg = getGhlConfig();
  const pipeline = await resolveEndOfFunnelPipeline();
  const stageId = kind === 'emailed' ? cfg.referralEmailSentStageId : cfg.referredStageId;
  const stageName = kind === 'emailed' ? cfg.referralEmailSentStageName : cfg.referredStageName;
  const stage = findStage(pipeline, stageId, stageName);
  if (!stage) {
    const err = new Error(`Could not find a "${stageName}" stage in the "${pipeline.name}" pipeline in GHL.`);
    err.statusCode = 400;
    throw err;
  }
...
// Add a tag to a contact (best-effort — never throws).
async function addContactTag(contactId, tag) {
  if (!contactId || !tag) return { skipped: true };
  try {
    const data = await ghlRequest(`/contacts/${encodeURIComponent(contactId)}/tags`, {
      method: 'POST',
      body: JSON.stringify({ tags: [tag] }),
    });
    return { ok: true, tag, data };
  } catch (error) {
    return { ok: false, tag, error: error.message, statusCode: error.statusCode || 500 };
  }
}

// After the review + referral email is sent: tag the contact "Review email
// sent" and move the opportunity to End of Funnel / "Referral Email Sent".
// Best-effort — a failure here is reported, it never fails the email send.
async function markReviewEmailSent(opportunity) {
  const cfg = getGhlConfig();
  const tag = await addContactTag(opportunity.contactId, cfg.reviewSentTag);
  let move;
  try {
    const dest = await resolveEndOfFunnelStage('emailed');
    await moveOpportunityToPipelineStage(opportunity.id, dest.pipelineId, dest.stageId);
    move = { moved: true, pipelineName: dest.pipelineName, stageName: dest.stageName, warning: '' };
  } catch (error) {
    move = { moved: false, pipelineName: '', stageName: '', warning: error.message };
  }
  return { tag, move };
}
...
async function linkReferral(target, referrer, note) {
  ...
  // Resolve the End of Funnel "Referred Opportunities" destination up front
  // (best-effort) so we can both record where each opportunity came from and
  // move them there.
  let dest = null;
  let moveWarning = '';
  try {
    dest = await resolveEndOfFunnelStage('referred');
  } catch (error) {
    moveWarning = error.message;
  }

  // Capture each opportunity's pre-move pipeline/stage so Unlink can restore
  // them. Keep any value already stored (survives re-links / "change referrer")
  // and never record the End-of-Funnel destination itself as the "previous".
  const atDest = (pipeId) => !!(dest && pipeId && String(pipeId) === String(dest.pipelineId));
  const targetPrevPipe = target.referralPreviousPipelineId || (atDest(target.pipelineId) ? '' : (target.pipelineId || ''));
  const targetPrevStage = target.referralPreviousStageId || (atDest(target.pipelineId) ? '' : (target.pipelineStageId || ''));
  const referrerPrevPipe = target.referrerPreviousPipelineId || (atDest(referrer.pipelineId) ? '' : (referrer.pipelineId || ''));
  const referrerPrevStage = target.referrerPreviousStageId || (atDest(referrer.pipelineId) ? '' : (referrer.pipelineStageId || ''));
  ...
  const referralUpdate = await updateOpportunityFields(target.id, {
    ...
    referralPreviousPipelineId: targetPrevPipe,
    referralPreviousStageId: targetPrevStage,
    referrerPreviousPipelineId: referrerPrevPipe,
    referrerPreviousStageId: referrerPrevStage,
  ...
  // Move both opportunities into End of Funnel / Completed (best-effort — a
  // failure here is reported as a warning and never rolls back the link).
  const moved = [];
  if (dest) {
    for (const id of [target.id, referrer.id].filter(Boolean)) {
      try {
  ...
  const pipelineMove = {
    moved,
    pipelineName: dest ? dest.pipelineName : '',
    stageName: dest ? dest.stageName : '',
    warning: moveWarning,
  };
...
async function unlinkReferral(target) {
  ...
  // Restore both opportunities to the pipeline/stage they were in before the
  // link moved them to End of Funnel (best-effort — only where we recorded it).
  const restored = [];
  let restoreWarning = '';
  try {
    if (target.referralPreviousPipelineId && target.referralPreviousStageId) {
      await moveOpportunityToPipelineStage(target.id, target.referralPreviousPipelineId, target.referralPreviousStageId);
      restored.push(target.id);
    }
    if (target.referredById && target.referrerPreviousPipelineId && target.referrerPreviousStageId) {
      await moveOpportunityToPipelineStage(target.referredById, target.referrerPreviousPipelineId, target.referrerPreviousStageId);
      restored.push(target.referredById);
    }
  } catch (error) {
    restoreWarning = error.message;
  }
  ...
  return {
    ...
    referralUpdate,
    pipelineRestore: { restored, warning: restoreWarning },
  };
}
...
module.exports = {
  ...
  moveOpportunityToPipelineStage,
  resolveEndOfFunnelStage,
  readJsonBody,
  requireTrackerAuth,
  resolveReferralFields,
  buildReviewEmail,
