...
async function resolveReferralFields() {
  const fields = {};
  const missing = [];
  const definitions = await fetchCustomFields();
...
async function getWonOpportunityTrackerData() {
  ...
  const won = opportunitiesRaw
    .filter((opportunity) => String(opportunity.status || '').toLowerCase() === 'won')
    .map((opportunity) => normalizeOpportunity(opportunity, fields, stageMap));
...
async function updateOpportunityFields(opportunityId, fieldValues) {
  ...
  const body = {
    locationId: existing.locationId || existing.location_id || cfg.locationId,
